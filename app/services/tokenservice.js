...
accessToken.findOne({token:req.body.access_token},function(err,data){
  if (err) {
    commonService.error(res, "Token is not Valid", {
      code: "400"
    });
  ...
  try {
    client.findOne({
      ...
      if (err) {
        ...
        users.findOne({$or:[{
          userName: req.body.username},{Id:req.body.username}]
        }, function(err, data) {
          if (data === null || data === "") {
            commonService.error(res, "User does not exist", {
              code: InputsError
            });
          } else if (err) {
            commonService.error(res, "Internal Server Error", {
